...
app.get('/api/spotify-home', async (req, res) => {
  ...
  try {
    const [releasesRes, recommendationsRes, artistsRes] = await Promise.all([
      fetch('https://api.spotify.com/v1/search?q=tag:new&type=album&limit=4', { headers }),
      fetch('https://api.spotify.com/v1/search?q=genre:pop&type=track&limit=6', { headers }),
      fetch('https://api.spotify.com/v1/search?q=genre:pop&type=artist&limit=6', { headers })
    ]);
    ...
    const formattedData = {
      releases: await Promise.all(releasesResult.albums.items.map(async (album) => {
        const tracksResponse = await fetch(`https://api.spotify.com/v1/albums/${album.id}/tracks`, { headers });
      ...
      artists: (artistsResult.artists?.items || artistsResult.items || []).map(artist => ({
...
app.get('/api/spotify-profile', async (req, res) => {
  ...
  try {
    const [profileRes, topTracksRes, followedArtistsRes] = await Promise.all([
      ...
      fetch('https://api.spotify.com/v1/me/following?type=artist&limit=6', { headers })
    ]);
